...
ESX.RegisterCommand('test', 'admin', function(xPlayer, args, showError)
    local start = os.nanotime()
    local xPlayers = ESX.GetExtendedPlayers()
    for i=1, #(xPlayers) do
        print(xPlayers[i].job.name)
    end
    print(('[es_extended] [^2INFO^7] (%.4f ms)'):format((os.nanotime() - start) / 1e6))
    Wait(3000)
    local startt = os.nanotime()
    local xPlayers = ESX.GetExtendedPlayers()
    for _, xPlayer in pairs(xPlayers) do
        print(xPlayer.job.name)
    end
    print(('[es_extended] [^2INFO^7] (%.4f ms)'):format((os.nanotime() - startt) / 1e6))
end, false, {help = _U('command_setcoords'), validate = true, arguments = {
	{name = 'z', help = 'Teszt', type = 'number'}
}})

ESX.RegisterCommand('s', 'superadmin', function(xPlayer, args, showError)
